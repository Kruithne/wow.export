...
class M2Exporter {
	 ...
	 * @param {boolean} raw
	 * @param {MTLWriter} mtl
	 * @param {boolean} fullTexPaths
	 */
	async exportTextures(out, raw = false, mtl = null, fullTexPaths = false) {
	...
	async exportAsGLTF(out) {
		...
		const textureMap = await this.exportTextures(out, false, null, true);
