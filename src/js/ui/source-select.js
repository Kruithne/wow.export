...
core.events.once('screen-source-select', async () => {
    ...
    const openInstall = async (installPath) => {
        try {
            ...
            core.setToast('error', util.format('It looks like %s is not a valid World of Warcraft installation.', selector.value), null, 10000);
    ...
    core.events.on('click-source-remote', () => {
        core.block(async () => {
            ...
            try {
                ...
                core.setToast('error', util.format('There was an error connecting to Blizzard\'s %s CDN, try another region!', tag.toUpperCase()), null, 10000);
