	...
	<div id="header" :class="{ shadowed: state.toast !== null }">
		...
		<div id="nav">
			...
			<context-menu @close="contextMenus.stateNavExtra = false" :node="contextMenus.stateNavExtra" id="menu-extra">
				...
				<span v-if="isDebugBuild" @click.self="reloadStylesheet" id="menu-extra-style">Reload Styling</span>
			</context-menu>
		</div>
	</div>
	...
	<div id="content">
		...
		<div id="config" v-if="screen === 'config'" :class="{ toastgap: toast !== null }">
			<div>
				...
				<file-field :value="configEdit.exportDirectory" :class="{ concern: isEditExportPathConcerning }"></file-field>
			</div>
			<div>
			...
			<div>
				...
				<menu-button class="spaced" :dropdown="true" :options="availableLocaleKeys" :default="selectedLocaleKey" @change="configEdit.cascLocale = $event"></menu-button>
			</div>
			<div>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-maps" v-if="screen === 'tab-maps'">
				<div class="list-container">
					<list-box id="listbox-maps" :selection="selectionMaps" :items="mapViewerMaps" :filter="userInputFilterMaps" :keyinput="true" :single="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="map"></list-box>
				</div>
				<div class="filter">
					<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
		...
		<keep-alive>
			<div class="tab" id="tab-items" v-if="screen === 'tab-items'">
				<div class="list-container">
					<item-list-box id="listbox-items" :selection="selectionItems" :items="listfileItems" :filter="userInputFilterItems" :keyinput="true" :includefilecount="true" unittype="item" @options="contextMenus.nodeItem = $event"></item-list-box>
				...
				<div id="items-sidebar" class="sidebar">
					...
					<checkbox-list :items="itemViewerTypeMask"></checkbox-list>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-models" v-if="screen === 'tab-models'">
				<div class="list-container">
					<list-box :selection="selectionModels" :items="listfileModels" :override="overrideModelList" :filter="userInputFilterModels" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="model"></list-box>
				</div>
				<div class="filter">
					<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
				...
				<div id="model-sidebar" class="sidebar">
					...
					<template v-if="modelViewerActiveType === 'm2'">
						...
						<checkbox-list :items="modelViewerGeosets"></checkbox-list>
						<div class="list-toggles">
						...
						<template v-if="config.modelsExportTextures">
							...
							<list-box-b :items="modelViewerSkins" :selection="modelViewerSkinsSelection" :single="true"></list-box-b>
						</template>
					</template>
					<template v-if="modelViewerActiveType === 'wmo'">
						<span class="header">WMO Groups</span>
						<checkbox-list :items="modelViewerWMOGroups"></checkbox-list>
						<div class="list-toggles">
							<a @click="setAllWMOGroups(true)">Enable All</a> / <a @click="setAllWMOGroups(false)">Disable All</a>
						</div>
						<span class="header">Doodad Sets</span>
						<checkbox-list :items="modelViewerWMOSets"></checkbox-list>
					</template>
				</div>
			</div>
		</keep-alive>
		<keep-alive>
			<div class="tab list-tab" id="tab-install" v-if="screen === 'tab-install'">
				<div class="list-container">
					<list-box :selection="selectionInstall" :items="listfileInstall" :filter="userInputFilterInstall" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="install file"></list-box>
				</div>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-raw" v-if="screen === 'tab-raw'">
				<div class="list-container">
					<list-box :selection="selectionRaw" :items="listfileRaw" :filter="userInputFilterRaw" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="file"></list-box>
				</div>
				<div id="tab-raw-tray">
					<div class="filter">
						<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-video" v-if="screen === 'tab-video'">
				<div class="list-container">
					<list-box :selection="selectionVideos" :items="listfileVideos" :filter="userInputFilterVideos" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="video"></list-box>
				</div>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-text" v-if="screen === 'tab-text'">
				<div class="list-container">
					<list-box :selection="selectionText" :items="listfileText" :filter="userInputFilterText" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="text file"></list-box>
				</div>
				<div class="filter">
					<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-sounds" v-if="screen === 'tab-sounds'">
				<div class="list-container">
					<list-box :selection="selectionSounds" :items="listfileSounds" :filter="userInputFilterSounds" :keyinput="true" :regex="config.regexFilters" :copymode="config.copyMode" :pasteselection="config.pasteSelection" :copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="true" unittype="sound file"></list-box>
				</div>
				<div class="filter">
					<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
				...
				<div id="sound-player">
					...
					<div id="sound-player-controls">
						...
						<slider-component id="slider-seek" v-model="soundPlayerSeek" @input="emit('click-sound-seek', $event)"></slider-component>
						<div class="buttons">
							<input type="button" :class="{ isPlaying: !soundPlayerState }" @click="click('sound-toggle', $event)"/>
							<slider-component id="slider-volume" v-model="config.soundPlayerVolume" @input="config.soundPlayerVolume = $event"></slider-component>
						</div>
					</div>
				</div>
				<div class="preview-controls">
		...
		<keep-alive>
			<div class="tab list-tab" id="tab-data" v-if="screen === 'tab-data'">
				<div class="list-container">
					<list-box :selection="selectionDB2s" :items="listfileDB2s" :filter="userInputFilterDB2s" :keyinput="true"
						:regex="config.regexFilters" :copydir="config.copyFileDirectories" :pasteselection="config.pasteSelection"
						:copytrimwhitespace="config.removePathSpacesCopy" :includefilecount="false" unittype="db2 file" :single="true"></list-box>
				</div>
				<div class="filter">
					<div class="regex-info" v-if="config.regexFilters" :title="regexTooltip">Regex Enabled</div>
