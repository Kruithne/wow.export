...
import * as CompressionType from './CompressionType.js';
...
class WDCReader {
	...
	async loadSchema(layoutHash) {
		...
		const tableName = ExportHelper.replaceExtension(basename(this.fileName));
		...
		if (structure === null) {
			const dbd_url = format(core.get_config('dbdURL'), tableName);
			const dbd_url_fallback = format(core.get_config('dbdFallbackURL'), tableName);
	...
	async parse() {
		...
		const data = await core.get_casc().getVirtualFileByName(this.fileName, false);
		this.data = data;

		this.castBuffer = BufferWrapper.alloc(8, true);

		const magic = data.readUInt32LE();
		const tableFormat = TABLE_FORMATS[magic];

		if (!tableFormat)
			throw new Error('Unsupported DB2 type: ' + magic);

		const wdcVersion = tableFormat.wdcVersion;
		log.write('Processing DB file %s as %s', this.fileName, tableFormat.name);
		...
		for (let sectionIndex = 0; sectionIndex < sectionCount; sectionIndex++) {
			...
			if (header.relationshipDataSize > 0) {
				...
				data.move(8); // relationshipMinID + relationshipMaxID
			...
			switch (recordFieldInfo.fieldCompression) {
				...
				case CompressionType.CommonData:
					if (this.commonData[fieldIndex].has(recordID))
						...
						out[prop] = recordFieldInfo.fieldCompressionPacking[0];
					break;
