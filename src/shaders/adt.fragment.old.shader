...
void main() {
	...
	vec3 tex1 = texture(pt_layer0, tc0).rgb;
	vec3 tex2 = texture(pt_layer1, tc1).rgb;
	vec3 tex3 = texture(pt_layer2, tc2).rgb;
	vec3 tex4 = texture(pt_layer3, tc3).rgb;

	vec4 mix = vec4(mixTextures(mixTextures(mixTextures(tex1, tex2, blendTex0), tex3, blendTex1), tex4, blendTex2), 1);
	fragColor = vec4(mix.rgb * vVertexColor.rgb * 2.0, 1);
}
